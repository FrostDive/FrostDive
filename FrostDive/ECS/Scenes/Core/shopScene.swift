...
class shopScene: SKScene {
    ...
    private func buildGrid() {
        ...
        let cW =
            (gridWidth - hSpacing * CGFloat(cardColumns - 1))
            / CGFloat(cardColumns)
        let cH =
            (gridHeight - vSpacing * CGFloat(cardRows - 1)) / CGFloat(cardRows)
    ...
    private func buildCard(
        ...
        if natSz.width > 0 && natSz.height > 0 {
            ...
            thumb.size = CGSize(
                width: natSz.width * sc,
                height: natSz.height * sc
            )
        } else {
            thumb.size = CGSize(
                width: maxThumbW * 0.85,
                height: maxThumbH * 0.85
            )
        }
    ...
    private func showConfirmPopup(for sub: SubmarineItem) {
        ...
        if imgNatSz.width > 0 && imgNatSz.height > 0 {
            ...
            img.size = CGSize(
                width: imgNatSz.width * sc,
                height: imgNatSz.height * sc
            )
        } else {
        ...
        confirmBtn.position = CGPoint(
            x: size.width / 2,
            y: size.height / 2 - popupH * 0.28
        )
    ...
    private func purchaseSubmarine(subIndex: Int) {
        guard let sub = submarines.first(where: { $0.index == subIndex }) else {
            return
        }
