...
extension gameScene {
    ...
    private func setupEdge() {
        let edgeFrame = CGRect(x: 0, y: 0, width: size.width, height: size.height)
    ...
    private func setupBackground() {
        ...
        for i in 0..<15 {
            ...
            let bg = backgroundEntity(imageName: imageName, size: self.size, startPosition: startPos, speed: bgSpeed, index: i)
            
            if let p = bg.component(ofType: positionComponent.self) { posSystem.addComponent(p) }
            if let m = bg.component(ofType: movementComponent.self) { moveSystem.addComponent(m) }
    ...
    private func spawnSubmarine() {
        ...
        let submarine = submarineEntity(imageName: "submarine1", size: CGSize(width: 120, height: 82), startPosition: startPos)
        
        // Simpan ke variabel playerEntity agar mudah diakses saat layar disentuh
        self.playerEntity = submarine
        
        // Daftarkan ke sistem
        if let t = submarine.component(ofType: thrustComponent.self) { thrustSys.addComponent(t) }
...
extension gameScene {
    ...
    private func spawnRandomEntity() {
        ...
        if isTrash {
            ...
            newEntity = trashEntity(imageName: trashImageName, size: trashSize, startPosition: startPos, speed: speed)
            ...
            case .iceberg1, .iceberg2:
                startPos = CGPoint(x: startX, y: size.height - halfHeight + overlapOffset)
            case .iceberg3, .iceberg4:
                startPos = CGPoint(x: startX, y: halfHeight - overlapOffset)
            }
            
            newEntity = obstacleEntity(imageName: randomObstacle.imageName, size: randomObstacle.size, startPosition: startPos, speed: speed)
        }
...
extension gameScene {
    ...
    private func cleanupOffScreenEntities() {
        ...
        entities.removeAll { entity in
            ...
            guard entity is trashEntity || entity is obstacleEntity else { return false }
            
            // 2. Cek apakah posisinya sudah jauh di luar layar kiri
            guard let posComp = entity.component(ofType: positionComponent.self),
                  posComp.position.x < -500 else { return false }
