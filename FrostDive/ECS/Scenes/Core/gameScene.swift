    ...
    func getStartPosition(sceneSize: CGSize, startX: CGFloat) -> CGPoint {
        ...
        case .bomb:
            ...
            let randomY = CGFloat.random(
                in: safeMargin...(sceneSize.height - safeMargin)
            )
...
class gameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if !hasSavedSession {
            ...
            sessionDistance +=
                dt * baseDistanceRate * Double(currentSpeedMultiplier)
            hud?.distance = Int(sessionDistance)
        }
...
extension gameScene {
    ...
    private func showTutorial() {
        ...
        let dimBackground = SKSpriteNode(
            color: UIColor.black.withAlphaComponent(0.5),
            size: self.size
        )

        dimBackground.position = CGPoint(
            x: size.width / 2,
            y: size.height / 2
        )

        dimBackground.zPosition = 250
        dimBackground.name = "tutorialBackground"

        addChild(dimBackground)

        enumerateChildNodes(withName: "//pauseButton") { node, _ in
            node.isHidden = true
        }
        
        let tutorial = SKSpriteNode(imageNamed: "attention")
    ...
    private func dismissTutorial() {
        ...
        gameScene.hasShownTutorialThisSession = true

        tutorialNode?.removeFromParent()
        tutorialNode = nil
        
        childNode(withName: "tutorialBackground")?.removeFromParent()
        enumerateChildNodes(withName: "//pauseButton") { node, _ in
            node.isHidden = false
        }
...
extension gameScene {
    ...
    private func finalizeSpawn(entity: GKEntity, startPos: CGPoint) {
        ...
        if let p = entity.component(ofType: positionComponent.self) {
            posSystem.addComponent(p)
        }
        if let m = entity.component(ofType: movementComponent.self) {
            moveSystem.addComponent(m)
        }
    ...
    private func spawnIcebergAlternating1(startX: CGFloat) {
        ...
        let types: [ObstacleType] = [
            .iceberg1, .iceberg3, .iceberg1, .iceberg3,
        ]
    ...
    private func spawnIcebergAlternating2(startX: CGFloat) {
        ...
        let types: [ObstacleType] = [
            .iceberg4, .iceberg2, .iceberg4, .iceberg2,
        ]
...
extension gameScene {
    ...
    private func cleanupOffScreenEntities() {
        ...
        entities.removeAll { entity in
            ...
            guard
                entity is trashEntity || entity is obstacleEntity
                    || entity is fishEntity
            else {
                return false
            }
